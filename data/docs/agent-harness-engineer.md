---
topic: LLM Agent 工程实践
keywords: agent loop, harness, context engineering, skills, memory, multi-agent, evaluation, observability, prompt injection, aci, tool design, openclaw
description: 深度介绍 LLM Agent 工程实践，包括控制流设计、Harness 测试基础设施、上下文分层管理、ACI 工具设计、记忆系统、多 Agent 协作协议、评测体系和安全边界，适合存放 Agent 工程落地相关知识
type: knowledge_base
---
## 1. 太长不读
...
---
## 补充知识（来自网络搜索）
> query: Harness架构 问题分析 解决方案 AI agent harness | 更新时间: 2026-04-11 | hash: 708d1eb2f718d1df0e04f46e6ef2bc72

# AI Agent Harness 架构解析

## 核心定义
**AI Agent Harness** 被比喻为 AI 时代的“缰绳与马鞍”，指代支撑 Agent 稳定运行的基础工程架构。它是决定 Agent 能否真正工作的基座工程，旨在将大模型能力转化为可落地的软件系统。

## 架构原理
- **控制与引导**：通过标准化接口约束 Agent 行为，防止幻觉与失控，确保任务执行的可控性。
- **能力封装**：将模型能力封装为可复用组件，实现与现有企业系统的无缝集成。
- **流程重构**：改变传统软件工程模式，利用 Agent 自动化完成开发与运维任务。

## 常见问题
- **不可靠性**：原生 LLM 输出随机，缺乏确定性逻辑。
- **集成壁垒**：Agent 难以适配复杂环境与遗留系统。
- **维护困难**：长链路任务缺乏状态追踪与错误恢复机制。

## 解决方案
- 构建统一 Harness 层，封装模型调用与工具链。
- 引入评估体系（Evaluation），持续监控 Agent 性能。
- 采用模块化设计，支持插件化扩展与动态配置。

## 来源参考
1. 知乎：AI Agent Harness 深度解读：AI Agent 时代的「缰绳与马鞍」
2. AI Agent 系统架构进阶指南：Agent Harness 深度解析
3. 基座工程（Harness Engineering）：决定你的 AI Agent 能否真正工作


---
## 补充知识（来自网络搜索）
> query: Harness engineering AI agent problem solution implementation framework | 更新时间: 2026-04-11 | hash: eabf4c190a5f60bd238609142f2ad9f4

# AI Agent 工程化与实施框架

## 核心概念
- **AI Agent Harness**: 用于编排和管理 AI 智能体的工程化系统，旨在解决复杂问题。
- **多智能体协作**: 利用多个 LLM 协同工作，处理单一模型难以完成的工程任务。

## 技术原理
- **向量检索**: 结合 Milvus 等向量数据库，增强 Agent 对上下文和外部知识的记忆与检索。
- **代码自动化**: 通过 AI Coding Agent 实现代码生成、审查与部署的自动化闭环。

## 实施框架
- **系统设计**: 构建支持 Agent 通信、状态管理及错误恢复的系统环境。
- **企业级应用**: 参考 Stripe、Shopify 等案例，将 Agent 嵌入现有开发流程。
- **评估体系**: 建立针对 Agent 输出质量与执行效率的评估标准。
...
---
## 补充知识（来自网络搜索）
> query: LangGraph components modules architecture 2024 | 更新时间: 2026-04-16 | hash: 3e35e852731fe75eb332e84c2dfd1335

## LangGraph 架构与核心组件

LangGraph 是 LangChain 生态中用于构建有状态、多行为体应用的框架。它通过图形结构定义代理的工作流，提供比线性链更强的灵活性和可控性，是构建复杂 AI Agent 的关键工具。

### 核心概念
- **节点**: 图中的执行单元，代表一个具体操作（如调用 LLM、使用工具）。
- **边**: 连接节点的路径，定义了工作流的走向和转换逻辑，可以是固定的或条件性的。
- **状态**: 在图中共享和传递的数据，为代理提供记忆和上下文，是其具备“有状态”能力的基础。

### 架构特点
- **图形化工作流**: 使用有向图而非简单的线性链，支持条件分支、循环和并行执行等复杂逻辑。
- **持久化状态**: 内置状态管理机制，可将 Agent 的中间状态持久化，便于恢复、调试和人机协作。
- **可控与可预测**: 精细化的流程控制使 Agent 行为更可靠、易于理解和干预。

来源:
- [LangGraph Architecture - Emergent Mind]()
- [LangGraph: Essential Components for Building AI Agents - Medium]()
- [LangGraph — Architecture and Design | Medium]()
- [Top 5 LangGraph Agents in Production 2024 - LangChain Blog]()


---
## 补充知识（来自网络搜索）
> query: "LangGraph" components list StateGraph MessageGraph RunnableConfig | 更新时间: 2026-04-16 | hash: 8a3b246ec302407ba5b9232d9087f4c5

## LangGraph 核心组件

- **StateGraph**: 基于“状态”的图。所有节点共享并操作一个中心化状态对象，适用于构建复杂的工作流。
- **MessageGraph**: 基于“消息”的图。是 StateGraph 的简化版，其状态仅是消息列表。节点间通过传递消息进行交互，更适合对话式Agent。
- **RunnableConfig**: 运行时配置对象。在调用 (`invoke`) 图或节点时传入，用于携带如线程ID、回调函数等上下文信息，与图的静态结构定义分离。

来源：
- [Prebuilt Components - LangGraph](URL：)
- [langgraph - LangChain Reference Docs](URL：)
- [Built with LangGraph! #4: Components | by Okan Yenigün - Medium](URL：)
- [RunnableConfig is an empty list when passed to invoke · Issue #5283](URL：)


---
## 补充知识（来自网络搜索）
> query: RAG升级到Agentic Search 实施步骤 | 更新时间: 2026-04-19

### 从 RAG 升级到 Agentic Search

*   **核心原理与对比**
    *   **传统 RAG**：采用线性“检索-生成”模式，依赖单次查询，缺乏对复杂问题的拆解与动态调整能力。
    *   **Agentic Search**：引入智能体机制，具备自主规划、多步推理和工具调用能力。它可根据任务反馈动态调整检索策略，实现从“静态问答”到“动态问题解决”的转变。

*   **实施步骤**
    1.  **构建规划能力**：利用大模型将复杂用户意图拆解为可执行的子任务链条。
    2.  **集成工具生态**：挂载搜索引擎、API 及数据库，赋予智能体多源信息获取能力。
    3.  **设计迭代机制**：建立“反思-重试”循环，对检索结果进行自我评估与修正。
    4.  **优化路由策略**：根据问题类型动态分发至不同的处理流，提升响应精准度。
